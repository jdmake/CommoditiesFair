...
;(function ($, window, undefined) {
    ...
    $(document).ready(function () {
        ...
        if (typeof Dropzone != 'undefined') {
            ...
            Dropzone.options.dictRemoveFile = "删除"
            $(".dropzone[action]").each(function (i, el) {
                var inputName = $(this).attr('name') || 'picture';
                ...
                $(el).dropzone({
                    ...
                    init: function () {
                        this.on("complete", function (file) {
                            var src = $(file.previewElement).find("[data-dz-thumbnail]").attr('src');
                            if (src.indexOf('data:image') <= -1) {
                                $(file.previewElement).append('<input type="hidden" value="' + src + '" name="form[' + inputName + '][]">');
                            }
                        });
                        this.emit("initimage", $(el).data('initimage'));
                        ...
                        this.on("success", function (file, data) {
                            ...
                            var res = data;
                            if (res.error === 0) {
                                $(file.previewElement).append('<input type="hidden" value="' + res.data.url + '" name="form[' + inputName + '][]">');
                            }
                        });
...
function bindChangeUploadFile(event, id) {
    var file = event.files[0];
    // 上传
    var data = new FormData();
    data.append('file', file);
    $.ajax({
        url: '/admin/fileupload/picture',
        type: 'post',
        cache: false,
        processData: false,
        contentType: false,
        ...
        success: function (res) {
            if (res.error > 0) {
                ...
                $(event).parent().parent().css({
                    background: "url('" + res.data.url + "') no-repeat center center"
                });
                $('#' + id).val(res.data.url);
            }
        },
...
// 多图上传
function multiplexUpload(event, id, name) {
    var file = event.files[0];
    // 上传
    var data = new FormData();
    data.append('file', file);
